...
class HDF5Writer(object):
    ...
    def __init__(self, filename, dataset_name, shape, dtype=np.uint8, attrs={}, mode="w", store_as_uint8=False):
        ...
        if self.dset.attrs["mode"] == "n_events":
            self.ts_dset = self.file.create_dataset(
                "last_timestamps", (self.dataset_size_increment,),
                maxshape=(None,),
                dtype=np.int64, compression='gzip', chunks=(1,))
    ...
    def write(self, array, last_timestamps=None):
        ...
        if self.dset.attrs["mode"] == "n_events":
            ...
            assert len(last_timestamps) == len(
                array), f"array and last_timestamps shapes must be the same: {len(last_timestamps)}, {len(array)}"
