...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Number of successive periods needed to confirm a vibration. Lower = more sensitive to weak/distant signals.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--min-pixel-count", dest="min_pixel_count", type=int, default=5,
        help="Minimum vibrating pixels to consider a frequency real (not noise). Lower = detect smaller/distant propellers.",
    )

    # FFT grid analysis
    parser.add_argument(
        "--grid-cells", dest="grid_cells", type=int, default=8,
        help="Number of grid cells per axis for ROI-based FFT analysis. Fewer = larger cells, better for distant objects.",
    )
    parser.add_argument(
        "--fft-window", dest="fft_window_sec", type=float, default=1.0,
        help="Rolling FFT window duration in seconds. Longer = better SNR for distant/weak signals.",
    )
    parser.add_argument(
        "--min-snr", dest="min_snr", type=float, default=2.0,
        help="Minimum signal-to-noise ratio for a cell's FFT peak to be reported. Lower = detect weaker/distant signals.",
    )
