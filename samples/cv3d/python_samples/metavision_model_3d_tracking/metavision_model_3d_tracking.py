...
def parse_args():
    ...
    detection_options.add_argument(
        '--detection-support-point-step', dest='detection_support_point', type=int, default=10,
        help='Distance (in pixels in the distorted image) between two support points')
    detection_options.add_argument(
        '--detection-search-radius', dest='detection_search_radius', type=int, default=10,
        help="Search radius for matches of each support point")
    detection_options.add_argument(
        '--flow-radius', dest='flow_radius', type=int, default=3,
        help="Radius used to estimate the normal flow of the edge")
    detection_options.add_argument(
        '--n-fitting-pts', dest='n_fitting_pts', type=int, default=3,
        help="Minimum number of matches required for line fitting")
    detection_options.add_argument(
        '--variance-threshold', dest='variance_threshold', type=float, default=2e-5,
        help="Threshold of the variance of the support points around the fitted line below which an edge is considered matched")
    detection_options.add_argument(
        '--fitted-edges-ratio', dest='fitted_edges_ratio', type=float, default=0.5,
        help="Matched edges to visible edges ratio above which a pose estimation is attempted")
    ...
    tracking_options.add_argument(
        '--tracking-search-radius', dest='tracking_search_radius', type=int, default=5,
        help="Search radius for matches of each support point")
    tracking_options.add_argument(
        '--tracking-support-point-step', dest='tracking_support_point', type=int, default=10,
        help="Distance (in pixels in the distorted image) between two support points")
    tracking_options.add_argument(
        '--n-last-poses', dest='n_last_poses', type=int, default=5,
        help="Number of past poses to consider when computing the accumulation time")
    tracking_options.add_argument(
        '--default-acc-time', dest='default_acc_time_us', type=int, default=5000,
        help="Default accumulation time used when the N last poses have not been estimated yet")
    tracking_options.add_argument('--oldest-weight', dest='oldest_weight', type=float,
                                  default=0.1, help="Weight attributed to oldest matches")
    tracking_options.add_argument(
        '--most-recent-weight', dest='most_recent_weight', type=float, default=1.,
        help="Weight attributed to most recent matches")
    tracking_options.add_argument(
        '--n-directional-axes', dest='nb_directional_axes', type=int, default=8,
        help="Number of axes used to discretize the possible directions of the edges' normals")
...
def main():
    ...
    detection_params = Model3dDetectionAlgorithm.Parameters()
    detection_params.support_point_step_ = args.detection_support_point
    detection_params.search_radius_ = args.detection_search_radius
    detection_params.flow_radius_ = args.flow_radius
    detection_params.n_fitting_pts_ = args.n_fitting_pts
    detection_params.variance_threshold_ = args.variance_threshold
    detection_params.fitted_edges_ratio_ = args.fitted_edges_ratio
    tracking_params = Model3dTrackingAlgorithm.Parameters()
    tracking_params.search_radius_ = args.tracking_search_radius
    tracking_params.support_point_step_ = args.tracking_support_point
    tracking_params.n_last_poses_ = args.n_last_poses
    tracking_params.default_acc_time_us_ = args.default_acc_time_us
    tracking_params.oldest_weight_ = args.oldest_weight
    tracking_params.most_recent_weight_ = args.most_recent_weight
    tracking_params.nb_directional_axes_ = args.nb_directional_axes

    # [INSTANTIATE_ALGOS_BEGIN]
    # Detection and Tracking algorithms
    detection_algo = Model3dDetectionAlgorithm(camera_geometry, model_3d, time_surface, detection_params)
    tracking_algo = Model3dTrackingAlgorithm(camera_geometry, model_3d, time_surface, tracking_params)
