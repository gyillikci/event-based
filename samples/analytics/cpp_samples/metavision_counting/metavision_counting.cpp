    ...
    if (!event_file_path_.empty()) {
        try {
            camera_ = std::make_unique<Metavision::Camera>(Metavision::Camera::from_file(
                event_file_path_, Metavision::FileConfigHints().real_time_playback(!replay_as_fast_as_possible_)));
...
bool Pipeline::parse_command_line(int argc, char *argv[]) {
    ...
    algo_options.add_options()
        ...
        ("min-y",       boost_po::value<int>(&min_y_line_)->default_value(120), "Ordinate at which to place the first line counter.")
        ("max-y",       boost_po::value<int>(&max_y_line_)->default_value(300), "Ordinate at which to place the last line counter.")
        ;
    // clang-format on
