...
public:
    ...
    /// @param out_avi_file Output file to save display to if specified
    VibrationGUIClass(int width, int height, float min_freq_hz, float max_freq_hz, float freq_precision_hz,
                      int min_pixel_count, const std::string &out_avi_file) :
        ...
        if (!out_avi_file.empty()) {
            video_writer_ =
                std::make_unique<cv::VideoWriter>(out_avi_file, cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 30,
                                                  cv::Size(width, freq_map_display_algo_->get_full_height()));
        }
...
private:
    ...
    void display_callback(Metavision::timestamp ts, const FrequencyMap &freqmap) {
        ...
        if (video_writer_)
            video_writer_->write(display_frame_);
...
bool parse_command_line(int argc, char *argv[], Config &config) {
    ...
    base_options.add_options()
        ...
        ("output-avi-file,o", po::value<std::string>(&config.out_avi_file_path_)->default_value(""), "Path to output AVI file to save the display video to. When the display is disabled, frames are not generated, so the video can't be generated either")
        ;
    // clang-format on
...
int main(int argc, char *argv[]) {
    ...
    if (conf.event_file_path_.empty()) {
        try {
        ...
        if (!conf.cam_config_path_.empty()) {
            try {
                camera.load(conf.cam_config_path_);
            } catch (const Metavision::CameraException &e) { MV_LOG_ERROR() << e.what(); }
        }
    } else {
        camera = Metavision::Camera::from_file(
            conf.event_file_path_, Metavision::FileConfigHints().real_time_playback(conf.realtime_playback_speed));
    }
    ...
    if (!conf.no_display) {
        vibration_gui = std::make_unique<VibrationGUIClass>(width, height, conf.vibration_config_.min_freq_,
                                                            ...
                                                            conf.min_pixel_count_, conf.out_avi_file_path_);
