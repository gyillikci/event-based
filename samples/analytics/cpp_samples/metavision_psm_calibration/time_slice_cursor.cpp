...
TimeSliceCursor::TimeSliceCursor(std::vector<EventCD> &&event_buffer, int precision_time, int buffer_duration) :
    ...
    update_parameters(precision_time, buffer_duration);
...
void TimeSliceCursor::update_parameters(int precision_time, int buffer_duration) {
    ...
    max_allowed_ts_ = precision_time * (max_events_ts_ / precision_time + buffer_duration + 1);
