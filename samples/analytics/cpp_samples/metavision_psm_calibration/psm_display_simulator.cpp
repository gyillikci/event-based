...
PsmDisplaySimulator::PsmDisplaySimulator(int width, int height, const PsmDisplaySimulatorConfig &config,
    ...
    csv_config_loader_->load_config(cluster_config_, particle_config_, precision_time_us_);

    // Initialize TimeSliceCursor (From events and config)
    timeslice_cursor_ =
        std::make_unique<TimeSliceCursor>(std::move(events), precision_time_us_, cluster_config_.bitsets_buffer_size_);

    // Initialize drawing helpers
    counting_drawing_helper_ = std::make_unique<CountingDrawingHelper>(rows);
    line_particle_drawing_helper_ =
        std::make_unique<LineParticleTrackDrawingHelper>(persistence_contour_ * precision_time_us_);
...
void PsmDisplaySimulator::run(RunType run_type) {
    if (run_type == RunType::FORWARD)
        ...
        // Reload config
        csv_config_loader_->load_config(cluster_config_, particle_config_, precision_time_us_);
        timeslice_cursor_->update_parameters(precision_time_us_, cluster_config_.bitsets_buffer_size_);

        const auto slice_condition = Metavision::EventBufferReslicerAlgorithm::Condition::make_n_us(precision_time_us_);
        reslicer_                  = std::make_unique<EventBufferReslicerAlgorithm>(
            [&](Metavision::EventBufferReslicerAlgorithm::ConditionStatus status, Metavision::timestamp ts,
                std::size_t n_events) {
                last_ts_ = ts;

                LineParticleTrackingOutput tracks;
                psm_algo_->get_results(ts, tracks, last_line_clusters_);
...
void PsmDisplaySimulator::process_events_and_update_image() {
    ...
    reslicer_->process_events(timeslice_cursor_->cbegin(), timeslice_cursor_->cend(),
                              [&](const auto &cbegin, const auto &cend) { psm_algo_->process_events(cbegin, cend); });
