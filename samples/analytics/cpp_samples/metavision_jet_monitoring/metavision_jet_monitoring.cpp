...
bool Pipeline::parse_command_line(int argc, char *argv[]) {
    ...
    input_options.add_options()
        ...
        ("input-event-file,i",    bpo::value<std::string>(&event_file_path_)->default_value(""), "Path to input event file (RAW or HDF5). If not specified, the camera live stream is used.")
        ("input-camera-config,j", bpo::value<std::string>(&cam_config_path_), "Path to a JSON file containing camera config settings to restore a camera state. Only works for live cameras.")
