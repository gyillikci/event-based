...
private:
    /// @brief Applies a filter only if enabled
    template<class FilterPtr>
    inline EventSlice apply_filter_if_enabled(const FilterPtr &filter, const EventSlice &slice, bool &is_first);
    ...
    void result_callback(const timestamp &ts, LineParticleTrackingOutput &tracks, LineClustersOutput &line_clusters);

    /// @brief Processing of the events coming from the camera
    void processing_callback(const Metavision::EventCD *begin, const Metavision::EventCD *end);
    ...
    LineParticleTrackingOutput tracks_; ///< Detected particles for each line
    LineClustersOutput line_clusters_;  ///< Detected clusters for each line
    ...
    std::unique_ptr<Metavision::EventBufferReslicerAlgorithm> slicer_;                  ///< Event slice producer
};
...
bool Pipeline::parse_command_line(int argc, char *argv[]) {
    ...
    base_options.add_options()
        ...
        ("input-event-file,i",    bpo::value<std::string>(&event_file_path_), "Path to input event file (RAW or HDF5). If not specified, the camera live stream is used.")
        ("input-camera-config,j", bpo::value<std::string>(&cam_config_path), "Path to a JSON file containing camera config settings to restore a camera state. Only works for live cameras.")
    ...
    detection_options.add_options()
        ...
        ("precision-time,p",        bpo::value<timestamp>(&precision_time_us_)->default_value(30), "Precision time in us (time duration between two asynchronous processes).")
        ("min-cluster-size",        bpo::value<int>(&cluster_ths_)->default_value(3), "Minimum width (in pixels) below which clusters of events along the line are considered as noise.")
        ...
        ("learning-rate",           bpo::value<float>(&learning_rate_)->default_value(0.8f), "Ratio in the weighted mean between the current x position and the observation."
                                                                                " This is used only when the particle is shrinking, because the front of the particle is always"
                                                                                " sharp while the trail might be noisy. 0.0 is conservative and does not take the observation"
                                                                                " into account, whereas 1.0 has no memory and overwrites the cluster estimate with the new observation.")
        ("clamping",                bpo::value<float>(&max_dx_allowed_)->default_value(5.f), "Caps x variation between cluster measurements at this value. A negative value disables the clamping."
                                                                                " This is used only when the particle is shrinking.")
        ;
    // clang-format on
    ...
    tracking_options.add_options()
        ...
        ("max-angle-deg",       bpo::value<int>(&max_angle_deg_)->default_value(45), "Maximum vertical angle that object trajectories can have. Beyond this angle,"
                                                                                " two particles on consecutive lines can't be matched.")
...
/// [RESULT_CALLBACK_BEGIN]
void Pipeline::result_callback(const timestamp &ts, LineParticleTrackingOutput &tracks,
...
struct EventSlice {
    Metavision::EventCD const *cbegin;
    Metavision::EventCD const *cend;
};

template<class FilterPtr>
inline EventSlice Pipeline::apply_filter_if_enabled(const FilterPtr &filter, const EventSlice &slice, bool &is_first) {
    if (filter == nullptr)
        return slice;

    if (is_first) {
        assert(slice.cbegin != nullptr && slice.cend != nullptr);
        buffer_filters_.clear();
        filter->process_events(slice.cbegin, slice.cend, std::back_inserter(buffer_filters_));
        is_first = false; // the next filters will have to use output_buffer instead of begin and end
    } else {
        auto end_it = filter->process_events(slice.cbegin, slice.cend, buffer_filters_.begin());
        buffer_filters_.resize(std::distance(buffer_filters_.begin(), end_it));
    }

    EventSlice output_slice;
    output_slice.cbegin = buffer_filters_.empty() ? nullptr : &buffer_filters_.front();
    output_slice.cend   = buffer_filters_.empty() ? nullptr : &buffer_filters_.back();

    return output_slice;
}

/// [PROCESSING_CALLBACK_BEGIN]
void Pipeline::processing_callback(const Metavision::EventCD *begin, const Metavision::EventCD *end) {
    if (!is_processing_)
        return;
    ...
    /// Apply filters
    bool is_first = true;
    EventSlice slice{begin, end};
    slice = apply_filter_if_enabled(polarity_filter_, slice, is_first);
    slice = apply_filter_if_enabled(transpose_events_filter_, slice, is_first);
    slice = apply_filter_if_enabled(activity_noise_filter_, slice, is_first);

    /// Process filtered events
    if (events_frame_generation_) {
        events_frame_generation_->process_events(slice.cbegin, slice.cend);
    }
    psm_tracker_->process_events(slice.cbegin, slice.cend);
}
/// [PROCESSING_CALLBACK_END]
...
bool Pipeline::initialize_trackers() {
    ...
    /// [SLICER_INIT_BEGIN]
    const auto condition = Metavision::EventBufferReslicerAlgorithm::Condition::make_n_us(precision_time_us_);
    slicer_              = std::make_unique<Metavision::EventBufferReslicerAlgorithm>(
        [&](Metavision::EventBufferReslicerAlgorithm::ConditionStatus, timestamp ts, std::size_t) {
            psm_tracker_->get_results(ts, tracks_, line_clusters_);
            result_callback(ts, tracks_, line_clusters_);
        },
        condition);
    /// [SLICER_INIT_END]
    ...
    Metavision::LineClusterTrackingConfig detection_config(bitsets_buffer_size, cluster_ths_, num_clusters_ths_,
                                                           min_inter_clusters_distance_, learning_rate_,
                                                           max_dx_allowed_, 0);
    ...
    if (!output_video_.empty() || !no_display_) {
        ...
        line_particle_drawing_helper_ = std::make_unique<Metavision::LineParticleTrackDrawingHelper>(
            static_cast<int>(persistence_contour_ * precision_time_us_));
        ...
        if (!no_display_) {
            window_ = std::make_unique<Metavision::Window>(
                "Particle Size Measurement Sample. Press 'q' or Escape key to exit.", total_width, sensor_height,
    ...
    camera_->cd().add_callback([this](const Metavision::EventCD *begin, const Metavision::EventCD *end) {
        slicer_->process_events(begin, end, [&](auto begin_it, auto end_it) { processing_callback(begin_it, end_it); });
    });
    /// [CAMERA_CALLBACK_END]
