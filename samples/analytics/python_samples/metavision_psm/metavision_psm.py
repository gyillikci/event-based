...
from metavision_sdk_analytics import PsmAlgorithm, LineClusterTrackingConfig, LineParticleTrackingConfig, \
    ...
    CountingDrawingHelper, LineParticleTrackDrawingHelper, LineClusterDrawingHelper, LineClustersOutputBuffer
...
def parse_args():
    ...
    detection_options.add_argument(
        '--min-cluster-size', dest='cluster_ths', type=int, default=3,
    ...
    detection_options.add_argument(
        ...
        type=float,
        default=5.0,
        help='Threshold that caps the x variation between cluster measurements at this value. A negative value disables the clamping. This is used only when the particle is shrinking.')
    ...
    tracking_options.add_argument(
        ...
        help="Maximum vertical angle that object trajectories can have. Beyond this angle,"
...
def main():
    ...
    if not (i_roi is None or args.roi_line_half_width is None):
        ...
        if args.rotate:  # Transpose
            ...
            i_roi.enable(True)
            print("Columns of interest: " + str(cols_rois))
        else:
            rows_rois = [1] * height
            for id in expanded_detection_rows:
                rows_rois[id] = 1
    ...
    mv_iterator = EventsIterator.from_device(
        ...
        if args.process_to else None, delta_t=args.precision_time)
    ...
    detection_config = LineClusterTrackingConfig(bitsets_buffer_size=int(args.accumulation_time / args.precision_time),
    ...
    # Output buffers
    tracks = LineParticleTrackingOutput()
    line_clusters = LineClustersOutputBuffer()
    ...
    if args.no_display:
        ...
        for evs in mv_iterator:
            ...
            if filtering_algorithms:
                ...
                psm_algo.process_events(events_buf, ts, tracks, line_clusters)
            else:
                psm_algo.process_events(evs, ts, tracks, line_clusters)
        ...
        tracking_drawing_helper = LineParticleTrackDrawingHelper(
            persistence_time_us=args.persistence_contour * args.precision_time)
        ...
        with MTWindow(title="Particle Size Measurement", width=width, height=height,
