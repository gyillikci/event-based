...
public:
    ...
    CmdLineOptions(int argc, char *argv[]) : options_desc_("Metavision XYT - Usage") {
        ...
        if (serial.size() && event_file_path.size()) {
            throw std::invalid_argument("Error : `--serial` and `--input-event-file` options are exclusives.");
        }
    }
    ...
    void preViewportUpdate(const Ogre::RenderTargetViewportEvent &evt) override {
        ...
        if (ImGui::BeginMainMenuBar()) {
            ...
            if (ImGui::BeginMenu("Settings")) {
                ...
                ImGui::Separator();
                ImGui::Text("Event Stream Software Filter");
...
public:
    ...
    std::unique_ptr<Metavision::Camera> make_camera() {
        ...
        if (opts_.event_file_path.size()) {
            ...
            camera = std::make_unique<Metavision::Camera>(
                Metavision::Camera::from_file(opts_.event_file_path, camera_fileconf));
            ...
            if (!opts_.cam_config_path.empty()) {
                camera->load(opts_.cam_config_path);
            }
        }

        camera->add_runtime_error_callback([](const Metavision::CameraException &e) { throw; });
    ...
    bool end_of_file() const {
        return camera_has_stopped && opts_.event_file_path.size();
    }
...
public:
    ...
    void setup() override {
        ...
        auto point_cloud_sliced_node =
            point_cloud_node_->createChildSceneNode(Ogre::Vector3(camera_width_ * -1.f, 0.f, 0.f));
