...
void ImGuiOverlay::ImGUIRenderable::createMaterial() {
    ...
    mPass->setSeparateSceneBlending(SBF_SOURCE_ALPHA, SBF_ONE_MINUS_SOURCE_ALPHA, SBF_ONE_MINUS_SOURCE_ALPHA, SBF_ZERO);
...
void ImGuiOverlay::ImGUIRenderable::createFontTexture() {
    ...
    mFontTex = TextureManager::getSingleton().createManual("ImGui/FontTex", RGN_INTERNAL, TEX_TYPE_2D, width, height, 1,
                                                           1, PF_BYTE_RGBA);
...
void ImGuiOverlay::NewFrame() {
    ...
    ImGuiIO &io = ImGui::GetIO();
    io.DeltaTime =
        std::max<float>(float(now - lastTime) / 1000,
...
void ImGuiOverlay::ImGUIRenderable::_update() {
    ...
    mXform = Matrix4(2.0f / (R - L), 0.0f, 0.0f, (L + R) / (L - R), 0.0f, -2.0f / (B - T), 0.0f, (T + B) / (B - T),
...
bool ImGuiOverlay::ImGUIRenderable::preRender(SceneManager *sm, RenderSystem *rsys) {
    ...
    for (int i = 0; i < draw_data->CmdListsCount; ++i) {
        ...
        for (int j = 0; j < draw_list->CmdBuffer.Size; ++j) {
            ...
            // Set scissoring
            Rect scissor(drawCmd->ClipRect.x, drawCmd->ClipRect.y, drawCmd->ClipRect.z, drawCmd->ClipRect.w);
...
void ImGuiOverlay::ImGUIRenderable::updateVertexData(const ImVector<ImDrawVert> &vtxBuf,
    ...
    if (bind->getBindings().empty() || bind->getBuffer(0)->getNumVertices() != size_t(vtxBuf.size())) {
        bind->setBinding(0, HardwareBufferManager::getSingleton().createVertexBuffer(sizeof(ImDrawVert), vtxBuf.size(),
                                                                                     HBU_CPU_TO_GPU));
    }
