...
struct SparseFlowFrameGeneration {
    ...
    using FlowBuffer  = std::vector<EventOpticalFlow>;
    ...
    void process_flow_events(std::vector<EventOpticalFlow> &flow_events) {
        try {
            if (!flow_events.empty()) {
                auto out_buffer = flow_buffer_pool_.acquire();
                std::swap(*out_buffer, flow_events);
                flow_buffers_.emplace(out_buffer);
                timestamp last_cd_ts = -1, last_flow_ts = -1;
                if (!cd_buffers_.empty() && !cd_buffers_.back()->empty())
                    last_cd_ts = cd_buffers_.back()->back().t;
                if (!flow_buffers_.empty() && !flow_buffers_.back()->empty())
...
private:
    ...
    void update_frame_with_cd(timestamp ts_begin, timestamp ts_end, cv::Mat &frame) {
        ...
        while (!cd_buffers_.empty()) {
            const auto &buffer = cd_buffers_.front();
            if (!buffer->empty()) {
    ...
    void update_frame_with_flow(timestamp ts_begin, timestamp ts_end, cv::Mat &frame) {
        while (!flow_buffers_.empty()) {
            const auto &buffer = flow_buffers_.front();
            if (!buffer->empty()) {
    ...
    SharedObjectPool<FlowBuffer> flow_buffer_pool_;
    std::queue<SharedObjectPool<FlowBuffer>::ptr_type> flow_buffers_;
