...
def train_parser():
    ...
    parser.add_argument('--allow_labels_interpolation', action="store_true",
                        help='interpolate labels if label_delta_t too small compared to frame delta_t')
...
def check_params(params):
    ...
    # check if the dataset is computed with n_events mode, in this case only feedforward models can be used
...
def train(params: argparse.Namespace):
    ...
    if params.just_demo:
        ...
        pl.Trainer(gpus=gpu_nums, precision=params.precision, limit_test_batches=params.limit_val_batches).test(
            model, dataloaders=classif_data.val_dataloader())
        ...
        pl.Trainer(gpus=gpu_nums, precision=params.precision, limit_test_batches=params.limit_test_batches).test(
            model, dataloaders=classif_data.test_dataloader())
    else:
