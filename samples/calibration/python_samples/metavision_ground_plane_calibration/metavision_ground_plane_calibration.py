...
def load_intrinsics(intrinsics_directory):
    ...
    intrinsics_json_file = os.path.join(intrinsics_directory, "intrinsics.json")
    if os.path.isfile(cam_file) or os.path.isfile(dist_file):
        ...
        assert os.path.isfile(intrinsics_json_file)
        fs = cv2.FileStorage(intrinsics_json_file, cv2.FILE_STORAGE_READ)
        assert fs.isOpened()
